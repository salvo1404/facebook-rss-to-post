...
class FbRssToPostListener
{
    ...
    function process()
    {
        ...
        if (!isset($_POST['info_update']) || !wp_verify_nonce($_POST['fb_rss_nonce'], 'settings_page')) {
        ...
        if (isset($_FILES['import_json']) && is_uploaded_file($_FILES['import_json']['tmp_name'])) {
            ...
            // - & -> &amp;
            //$json = preg_replace( '/(&(?!amp;))/', '&amp;', $json );

            $engine = new FbRssToPostEngine();
